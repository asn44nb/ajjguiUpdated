...
execute store result score @s ajjgui at @s positioned ~-0.75 ~-0.5 ~-0.75 if entity @a[gamemode=!spectator,dx=0.5,dy=0,dz=0.5]
execute unless score @s ajjgui matches 1 run function ajjgui:gui/ported/close

execute on controller on vehicle run scoreboard players set @s ajjgui 1
execute unless score @s ajjgui matches 1 run function ajjgui:gui/ported/close
